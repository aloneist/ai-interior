# 010 - Build TypeScript Fix for recommend-space

## Goal
Fix the current TypeScript build error in `app/api/recommend-space/route.ts` without changing recommendation behavior.

This task is for a narrow type-correctness fix only.
Do not change scoring, ranking, filtering, prompt logic, fallback behavior, response shape, or request semantics.
...
## Primary Objective
- Resolve the current build-blocking TypeScript error in `app/api/recommend-space/route.ts`
- Make the local types accurately reflect the actual data shape used in this file
- Keep the diff as small as possible
- Preserve runtime behavior exactly

## Known Problem
The current build error indicates that a cast like `vectors as FurnitureVectorRow[]` is not type-safe because the nested `furniture` field shape does not match what `FurnitureVectorRow` currently expects.
...
- Make minimal type-only changes required to satisfy TypeScript
...
## Completion Criteria
- The current TypeScript error in `app/api/recommend-space/route.ts` is resolved
...
1. Review the diff and confirm only `app/api/recommend-space/route.ts` was changed
