...
Operator-friendly runtime bridge commands:
...
`npm run automation:runtime:inspect` now prints both a final `FINAL SUMMARY`
block for operators and a `JSON REPORT` block derived from the same in-memory
inspection result. If `AUTOMATION_RUNTIME_INSPECT_REPORT_PATH` is set, the
same JSON report is also written to that file and CI preserves it as the
`automation-runtime-inspect-report` artifact. Route behavior itself is
unchanged.

Boundary note:
- `npm run automation:runtime:http:verify` is the assertion-focused command for correctness and safety boundaries
- `npm run automation:runtime:inspect` is the report-focused command for current-state inspection, summary, and JSON output

The three runtime report producers now share a more consistent outer envelope:
- `generatedAt`
- `command`
- `reportKind`
- `purpose`
...
`npm run automation:runtime:http:verify` now also prints a final `FINAL SUMMARY`
block and a `JSON REPORT` block derived from one in-memory verifier result. If
`AUTOMATION_RUNTIME_VERIFY_REPORT_PATH` is set, the same verifier JSON report
is also written to that file and CI preserves it as the
`automation-runtime-http-verify-report` artifact. The existing verifier text
log artifact remains in place.

`npm run automation:runtime:check` reuses the existing inspect and verifier
JSON reports, prints one combined `FINAL SUMMARY`, and emits one combined
`JSON REPORT`. If `AUTOMATION_RUNTIME_CHECK_REPORT_PATH` is set, the same
combined JSON report is also written to that file and CI preserves it as the
`automation-runtime-check-report` artifact.

When `AUTOMATION_RUNTIME_ARTIFACT_MANIFEST_PATH` is set during
`npm run automation:runtime:check`, that same combined flow also writes one
runtime artifact manifest built from the existing inspect, verify, and check
report outputs. CI preserves it as the `automation-runtime-artifact-manifest`
artifact. Route behavior itself is unchanged.

`npm run automation:readiness:report` builds one combined automation readiness
report from existing smoke and runtime report outputs only. If
`AUTOMATION_READINESS_REPORT_PATH` is set, the same readiness JSON report is
written to that file and CI preserves it as the
`automation-readiness-report` artifact. This does not broaden smoke or runtime
verification scope.

For final baseline review, use:
- [final-review.md](/workspaces/ai-interior/automation/final-review.md)

That final-review bundle treats the current smoke, runtime, manifest, and
readiness artifacts as the reviewable near-complete automation baseline. Route
